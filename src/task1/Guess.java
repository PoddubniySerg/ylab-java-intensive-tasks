...
public class Guess {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= maxAttempts; i++) {
            ...
            if (input > number) System.out.printf("Мое число меньше! У тебя осталось %d попыток\n", maxAttempts - i);
            if (input < number) System.out.printf("Мое число больше! У тебя осталось %d попыток\n", maxAttempts - i);
            if (input == number) {
